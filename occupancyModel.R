  ...
  for(i in 1:nSite){
    for (k in 2:nYear){
      ...
      lgamma[i, k - 1] <- alpha.gamma + 
        ...
        beta.highElevGamma * highElev[i, k - 1] +
        beta.meanElevGamma * meanElev[i, k - 1] +
  ...
  for (i in 1:nSite){
    for(j in 1: nRep){
      for (k in 1:nYear){
        ...
        lp.lim[i,j,k] <- min(999, max(-999, lp[i, j, k])) # Center pl value betwenn -999 and 999
        lp[i, j, k] <- alpha.p +
          beta.roads * cellRoadCov[i,k] +
...
mydata <- list(y = y,
               ...
               shortDisp = yShortDisp, # error when scaling this one
...
occWolfMCMC <- buildMCMC(occWolfConf, useConjugacy = FALSE) # second tip
