...
altiCov[is.na(altiCov$meanAlt), "meanAlt"] <- 0
...
# Culling
# Number of wolves legally killed per grid cells each year
cullData <- read.csv("data/wolfData/prelevement_2011_2020.csv", header = TRUE, sep = ";")
# Select data with a date and coordinates
culledWolves <- cullData[cullData$DATE != "" & !is.na(cullData$DATE) & !is.na(cullData$X) & !is.na(cullData$Y), 1:15] # 379 observations
# Remove the few poaching data
culledWolves <- culledWolves[culledWolves$MODALITE != "Braconnage",] # 366 observations
# Coordinate system for the locations is RGF93/lambert 93
# +proj=lcc +lat_1=49 +lat_2=44 +lat_0=46.5 +lon_0=3 +x_0=700000 +y_0=6600000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs
culledWolvesSP <- SpatialPointsDataFrame(coords = culledWolves[ ,c("X", "Y")], data = culledWolves, 
                                   proj4string = CRS(as.character("+proj=lcc +lat_1=49 +lat_2=44 +lat_0=46.5 +lon_0=3 +x_0=700000 +y_0=6600000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")),
                                   match.ID = TRUE, bbox = NULL)
library(lubridate)
# Deal with date of different formats
culledWolvesSP$DATE <- parse_date_time(culledWolvesSP$DATE, orders = c('Ymd', 'dmY'))
culledWolvesSP$DATE <- as.Date(culledWolvesSP$DATE, format = "%Y-%m-%d")

# How many culled wolves each year from April Y to March Y+1
yearStart <- 1993 # start of the wolf monitoring
yearEnd <- 2020 # winter 2020/2021 is the last available with validated data
culling <- matrix(data = 0, nrow = length(gridFr), ncol = yearEnd-yearStart + 1)

for(year in yearStart:yearEnd){
  
  # Extract the data for the time period
  cullYear <- culledWolvesSP[culledWolvesSP$DATE >= as.Date(paste0(year, "-04-01"), format = "%Y-%m-%d") & 
                              culledWolvesSP$DATE < as.Date(paste0(year+1, "-04-01"), format = "%Y-%m-%d"),]
  
  # Find in which cells cullings were made
  if(length(cullYear) != 0){
    cullYearCells <- over(cullYear, gridFr)
    nCullYearCells <- table(cullYearCells$ID)
    culling[as.numeric(names(nCullYearCells)), year - yearStart + 1] <- nCullYearCells
  }

}
save(culling, file = "data/culling.RData")
